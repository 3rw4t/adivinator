...
getRemainingProperties(db_playing).then(a => {document.getElementById("test").innerHTML =  a;})
async function boton(respuesta) {
	await filter(respuesta);
	document.getElementById("pregunta").innerHTML =  await getPregunta();
	document.getElementById("test").innerHTML =  await getRemainingProperties(db_playing); 
}
